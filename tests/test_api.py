...
from sanic_restful.utils import http_status_message, unpack
import sanic_restful
import sanic_restful.fields
from sanic_restful import OrderedDict
...
class HelloWorld(sanic_restful.Resource):
    def get(self):
        return {}
...
class HelloBomb(sanic_restful.Resource):
    def get(self):
...
class APITestCase(unittest.TestCase):
    ...
    def test_unauthorized(self):
        app = Flask(__name__)
        api = sanic_restful.Api(app, serve_challenge_on_401=True)
        response = Mock()
        response.headers = {}
        with app.test_request_context('/foo'):
            response = api.unauthorized(response)
    ...
    def test_handle_error_does_not_swallow_custom_exceptions(self):
        ...
        api = sanic_restful.Api(app, errors=errors)
    ...
    def test_marshal(self):
        fields = OrderedDict([('foo', sanic_restful.fields.Raw)])
        marshal_dict = OrderedDict([('foo', 'bar'), ('bat', 'baz')])
        output = sanic_restful.marshal(marshal_dict, fields)
        self.assertEquals(output, {'foo': 'bar'})

    def test_marshal_with_envelope(self):
        fields = OrderedDict([('foo', sanic_restful.fields.Raw)])
        marshal_dict = OrderedDict([('foo', 'bar'), ('bat', 'baz')])
        output = sanic_restful.marshal(marshal_dict, fields, envelope='hey')
        self.assertEquals(output, {'hey': {'foo': 'bar'}})

    def test_marshal_decorator(self):
        fields = OrderedDict([('foo', sanic_restful.fields.Raw)])

        @sanic_restful.marshal_with(fields)
        def try_me():
    ...
    def test_marshal_decorator_with_envelope(self):
        ...
        @sanic_restful.marshal_with(fields, envelope='hey')
        def try_me():
            return OrderedDict([('foo', 'bar'), ('bat', 'baz')])
    ...
    def test_marshal_field_decorator(self):
        field = sanic_restful.fields.Raw

        @sanic_restful.marshal_with_field(field)
        def try_me():
            return 'foo'
    ...
    def test_marshal_field(self):
        ...
        output = sanic_restful.marshal(marshal_fields, fields)
        self.assertEquals(output, {'foo': 'bar'})
    ...
    def test_marshal_tuple_with_envelope(self):
        fields = OrderedDict({'foo': sanic_restful.fields.Raw})
        marshal_fields = OrderedDict([('foo', 'bar'), ('bat', 'baz')])
        output = sanic_restful.marshal((marshal_fields,), fields, envelope='hey')
        self.assertEquals(output, {'hey': [{'foo': 'bar'}]})

    def test_marshal_nested(self):
        fields = OrderedDict([
            ('foo', sanic_restful.fields.Raw),
            ('fee', sanic_restful.fields.Nested({
                'fye': sanic_restful.fields.String,
            }))
        ])
    ...
    def test_marshal_nested_with_non_null(self):
        fields = OrderedDict([
            ...
            ('fee', sanic_restful.fields.Nested(
                OrderedDict([
                    ('fye', sanic_restful.fields.String),
                    ('blah', sanic_restful.fields.String)
                ]), allow_null=False))
        ])
    ...
    def test_marshal_nested_property(self):
        ...
        obj.foo = 'bar'
        obj.bat = 'baz'
        output = sanic_restful.marshal([obj], fields)
        expected = [OrderedDict([('foo', 'bar'), ('fee', OrderedDict([('fye', None), ('blah', 'cool')]))])]
        self.assertEquals(output, expected)
    ...
    def test_marshal_list(self):
        fields = OrderedDict([
            ('foo', sanic_restful.fields.Raw),
            ('fee', sanic_restful.fields.List(sanic_restful.fields.String))
        ])
        marshal_fields = OrderedDict([('foo', 'bar'), ('bat', 'baz'), ('fee', ['fye', 'fum'])])
        output = sanic_restful.marshal(marshal_fields, fields)
    ...
    def test_marshal_list_of_nesteds(self):
        fields = OrderedDict([
            ('foo', sanic_restful.fields.Raw),
            ('fee', sanic_restful.fields.List(sanic_restful.fields.Nested({
                'fye': sanic_restful.fields.String
            })))
        ])
        marshal_fields = OrderedDict([('foo', 'bar'), ('bat', 'baz'), ('fee', {'fye': 'fum'})])
    ...
    def test_marshal_list_of_lists(self):
        fields = OrderedDict([
            ('foo', sanic_restful.fields.Raw),
            ('fee', sanic_restful.fields.List(sanic_restful.fields.List(
                sanic_restful.fields.String)))
        ])
        marshal_fields = OrderedDict([('foo', 'bar'), ('bat', 'baz'), ('fee', [['fye'], ['fum']])])
        output = sanic_restful.marshal(marshal_fields, fields)
        expected = OrderedDict([('foo', 'bar'), ('fee', [['fye'], ['fum']])])
    ...
    def test_marshal_nested_dict(self):
        fields = OrderedDict([
            ('foo', sanic_restful.fields.Raw),
            ('bar', OrderedDict([
                ('a', sanic_restful.fields.Raw),
                ('b', sanic_restful.fields.Raw),
            ])),
        ])
    ...
    def test_api_prefix(self):
        app = Mock()
        app.configure_mock(**{'record.side_effect': AttributeError})
        api = sanic_restful.Api(app, prefix='/foo')
    ...
    def test_handle_api_error(self):
        ...
        class Test(sanic_restful.Resource):
            def get(self):
                flask.abort(404)
    ...
    def test_non_api_error_404_catchall(self):
        app = Flask(__name__)
        api = sanic_restful.Api(app, catch_all_404s=True)
        app = app.test_client()
    ...
    def test_add_two_conflicting_resources_on_same_endpoint(self):
        ...
        class Foo1(sanic_restful.Resource):
            def get(self):
                return 'foo1'

        class Foo2(sanic_restful.Resource):
            def get(self):
                return 'foo2'
    ...
    def test_resource(self):
        app = Flask(__name__)
        resource = sanic_restful.Resource()
        resource.get = Mock()
        with app.test_request_context("/foo"):
    ...
    def test_abort_data(self):
        try:
            sanic_restful.abort(404, foo='bar')
    ...
    def test_abort_custom_message(self):
        try:
            sanic_restful.abort(404, message="no user")
            assert False  # We should never get here
        except Exception as e:
            assert_equals(e.data['message'], "no user")

    def test_abort_type(self):
        self.assertRaises(HTTPException, lambda: sanic_restful.abort(404))
    ...
    def test_use_custom_jsonencoder(self):
        ...
        class Cabbage(sanic_restful.Resource):
            def get(self):
                return {'frob': object()}
    ...
    def test_redirect(self):
        ...
        class FooResource(sanic_restful.Resource):
            def get(self):
                return redirect('/')
    ...
    def test_json_float_marshalled(self):
        ...
        class FooResource(sanic_restful.Resource):
            fields = {'foo': sanic_restful.fields.Float}
            def get(self):
                return sanic_restful.marshal({"foo": 3.0}, self.fields)
    ...
    def test_selectively_apply_method_decorators(self):
        ...
        class TestResource(sanic_restful.Resource):
